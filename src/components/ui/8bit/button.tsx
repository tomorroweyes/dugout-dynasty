...
function Button({ children, asChild, ...props }: BitButtonProps) {
  ...
  return (
    <ShadcnButton
      {...props}
      className={cn(
        retroEnabled && "rounded-none active:translate-y-1 transition-transform",
        "relative inline-flex items-center justify-center gap-1.5 border-none m-1.5",
        size === "icon" && "mx-1 my-0",
        font !== "normal" && "retro",
        className
      )}
    ...
    >
      {asChild ? (
        <span className="relative inline-flex items-center justify-center gap-1.5">
          ...
          {retroEnabled && variant !== "ghost" && variant !== "link" && size !== "icon" && (
            <>
              {/* Pixelated border */}
